...
- Avoid mixing different components with the same specifications. The software is not able to "read" the labeling and will mark them even if they have the same specifications. If a component has changed during production, make a new REFERENCE image to avoid false positive marking.
...
- Ensure the image has been taken correctly focused and all components look sharp. If not, repeat the capture.
...
- Do not mix soldered and pre-soldered PCBAs. If you take a picture of a REFERENCE image with soldered components, do not inspect unsoldered UUI PCBAs, as doing so will result in a very high false positive rate.
